import {Injectable} from '@angular/core';
import {environment} from '../../../environments/environment';
import {HttpClient} from '@angular/common/http';
import {Observable} from 'rxjs';
import {Customer} from '../../model/customers/customer';

const API_URL = `${environment.apiUrl}`;

@Injectable({
  providedIn: 'root'
})
export class CustomerService {

  constructor(private http: HttpClient) {
  }

  getAll(): Observable<Customer[]> {
    return this.http.get<Customer[]>(API_URL + '/customer');
  }
  saveCustomer(customer): Observable<Customer> {
    return this.http.post<Customer>(API_URL + '/customer', customer);
  }
  search(textSearch: string): Observable<any> {
    return this.http.get(API_URL + '/customer' + '?customerName_like=' + textSearch);
  }
  // search(textSearch: string, typeSearch: any): Observable<any> {
  //   return this.http.get(API_URL + '/customer' + '?customerName_like=' + textSearch + '&customerTypeName_like=' + typeSearch);
  // }
  findById(id): Observable<Customer> {
    return this.http.get(`${API_URL}/customer/${id}`);
  }

  updateCustomer(id: number, customer: Customer): Observable<Customer> {
    return this.http.put<Customer>(`${API_URL}/customer/${id}`, customer);
  }

  deleteCustomer(id: number): Observable<Customer> {
    return this.http.delete<Customer>(`${API_URL}/customer/${id}`);
  }
}
